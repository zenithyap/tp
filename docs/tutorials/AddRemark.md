...
This command allows users of the Library application to add optional remarks to people in their author book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/author/logic/command` directory.
...
``` java
package seedu.author.logic.commands;

import seedu.author.model.Model;

/**
 * Changes the remark of an existing bookmark in the author book.
 */
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `LibraryParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.

You can refer to the changes in this [diff](https://github.com/se-edu/Library-level3/commit/35eb7286f18a029d39cb7a29df8f172a001e4fd8#diff-399c284cb892c20b7c04a69116fcff6ccc0666c5230a1db8e4a9145def8fa4ee).
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the bookmark identified "
            + "by the index number used in the last bookmark listing. "
...
``` java
import static seedu.author.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
    ...
    /**
     * @param index of the bookmark in the filtered bookmark list to edit the remark
     * @param remark of the bookmark to be updated to
     */
...
Your code should look something like [this](https://github.com/se-edu/Library-level3/commit/dc6d5139d08f6403da0ec624ea32bd79a2ae0cbf#diff-a8e35af8f9c251525063fae36c9852922a7e7195763018eacec60f3a4d87c594) after you are done.

### Parse user input

Now let’s move on to writing a parser that will extract the index and remark from the input provided by the user.

Create a `RemarkCommandParser` class in the `seedu.author.logic.parser` package. The class must extend the `Parser` interface.
...
:information_source: Don’t forget to update `LibraryParser` to use our new `RemarkCommandParser`!

</div>

If you are stuck, check out the sample
[here](https://github.com/se-edu/Library-level3/commit/dc6d5139d08f6403da0ec624ea32bd79a2ae0cbf#diff-8bf239e8e9529369b577701303ddd96af93178b4ed6735f91c2d8488b20c6b4a).

## Add `Remark` to the model

Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of bookmark data. We achieve that by working with the `Bookmark` model. Each field in a Bookmark is implemented as a separate class (e.g. a `Name` object represents the bookmark’s title). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a bookmark.

### Add a new `Remark` class

Create a new `Remark` in `seedu.author.model.bookmark`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.

A copy-paste and search-replace later, you should have something like [this](https://github.com/se-edu/Library-level3/commit/4516e099699baa9e2d51801bd26f016d812dedcc#diff-41bb13c581e280c686198251ad6cc337cd5e27032772f06ed9bf7f1440995ece). Note how `Remark` has no constrains and thus does not require input
validation.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each bookmark.

Simply add the following to [`seedu.author.ui.BookmarkCard`](https://github.com/se-edu/Library-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/BookmarkListCard.fxml`](https://github.com/se-edu/Library-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).

**`BookmarkListCard.fxml`:**
...
## Modify `Bookmark` to support a `Remark` field

Since `BookmarkCard` displays data from a `Bookmark`, we need to update `Bookmark` to get our `Remark` displayed!

### Modify `Bookmark`

We change the constructor of `Bookmark` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Bookmark`

Unfortunately, a change to `Bookmark` will cause other commands to break, you will have to modify these commands to use the updated `Bookmark`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Bookmark` class to find these commands.

</div>

Refer to [this commit](https://github.com/se-edu/Library-level3/commit/ce998c37e65b92d35c91d28c7822cd139c2c0a5c) and check that you have got everything in order!


## Updating Storage

Library stores data by serializing `JsonAdaptedBookmark` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedBookmark` to work with our new `Bookmark`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete Library’s storage file located at `/data/Library.json` before running it! Not doing so will cause Library to default to an empty author book!

</div>

Check out [this commit](https://github.com/se-edu/Library-level3/commit/556cbd0e03ff224d7a68afba171ad2eb0ce56bbf)
to see what the changes entail.

## Finalizing the UI

Now that we have finalized the `Bookmark` class and its dependencies, we can now bind the `Remark` field to the UI.

Just add [this one line of code!](https://github.com/se-edu/Library-level3/commit/5b98fee11b6b3f5749b6b943c4f3bd3aa049b692)
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Bookmark`. Since all fields in a `Bookmark` are immutable, we create a new instance of a `Bookmark` with the values that we want and
save it with `Model#setBookmark()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Bookmark: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Bookmark: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Bookmark editedBookmark = new Bookmark(
                BookmarkToEdit.getName(), BookmarkToEdit.getPhone(), BookmarkToEdit.getEmail(),
                BookmarkToEdit.getAddress(), remark, BookmarkToEdit.getTags());

        model.setBookmark(BookmarkToEdit, editedBookmark);
        model.updateFilteredBookmarkList(PREDICATE_SHOW_ALL_Bookmarks);

        return new CommandResult(generateSuccessMessage(editedBookmark));
    }
     ...
     * {@code BookmarkToEdit}.
     */
    private String generateSuccessMessage(Bookmark BookmarkToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, BookmarkToEdit);
    }
```
...
Following convention, let’s change the title of the generated method to `execute_addRemarkUnfilteredList_success`.
...
You should end up with a test that looks something like [this](https://github.com/se-edu/Library-level3/commit/fac8f3fd855d55831ca0cc73313b5943d49d4d6e#diff-ff58f7c10338b34f76645df49b71ecb2bafaf7611b20e7ff59ebc98475538a01R36-R49).

## Conclusion

This concludes the tutorial for adding a new `Command` to Library.
