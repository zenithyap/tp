...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `seedu.author.logic.Logic`.
...
A quick look at the `seedu.author.logic.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
...
1. _Step into_ the line where user input in parsed from a String to a Command, which should bring you to the `LibraryParser#parseCommand()` method (partial code given below):
   ``` java
...
1. The rest of the method seems to exhaustively check for the existence of each possible parameter of the `edit` command and store any possible changes in an `EditBookmarkDescriptor`. Recall that we can verify the contents of `editBookmarkDesciptor` through the 'Variables' window.<br>
...
1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setBookmark()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the bookmark data.
   * it uses the `updateFilteredBookmarkList` method to ask the `Model` to populate the 'filtered list' with _all_ bookmarks.<br>
     FYI, The 'filtered list' is the list of bookmarks resulting from the most recent operation that will be shown to the user immediately after. For the `edit` command, we populate it with all the bookmarks so that the user can see the edited bookmark along with all other bookmarks. If this was a `find` command, we would be setting that list to contain the search results instead.<br>
     To provide some context, given below is the class diagram of the `Model` component. See if you can figure out where the 'filtered list' of bookmarks is being tracked.
...
1. Similar to before, you can step over/into statements in the `LogicManager#execute()` method to examine how the control is transferred to the `Storage` component and what happens inside that component.
   ...
   <div markdown="span" class="alert alert-primary">:bulb: **Intellij Tip:** When trying to step into a statement such as `storage.saveLibrary(model.getLibrary())` which contains multiple method calls, Intellij will let you choose (by clicking) which one you want to step into.
   </div>

1.  As you step through the code inside the `Storage` component, you will eventually arrive at the `JsonLibrary#saveLibrary()` method which calls the `JsonSerializableLibrary` constructor, to create an object that can be _serialized_ (i.e., stored in storage medium) in JSON format. That constructor is given below (with added line breaks for easier readability):

    **`JsonSerializableLibrary` constructor:**
    ``` java
    /**
     * Converts a given {@code ReadOnlyLibrary} into this class for Jackson use.
     *
     * @param source future changes to this will not affect the created
     * {@code JsonSerializableLibrary}.
     */
    public JsonSerializableLibrary(ReadOnlyLibrary source) {
        bookmarks.addAll(
            source.getBookmarkList()
                  .stream()
                  .map(JsonAdaptedBookmark::new)
                  .collect(Collectors.toList()));
    }
    ```

1. It appears that a `JsonAdaptedBookmark` is created for each `Bookmark` and then added to the `JsonSerializableLibrary`.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the author book in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Bookmark`

    6.  Add a new entity to the author book
